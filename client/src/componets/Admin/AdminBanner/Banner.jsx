...
function Banner() {
 ...
 // dataBanner: lưu trữ danh sách các banner.
//isOpen: xác định xem drawer có đang mở hay không.
//image: lưu trữ ảnh được chọn để tạo banner mới.
  const fetchbanner = async () => {
    const res = await getBanner();
    setDataBanner(res?.response);
  };
 // fetchbanner: hàm async để lấy danh sách banner từ API và cập nhật state dataBanner.
//Sử dụng useEffect để gọi fetchbanner khi component được mount lần đầu.
  useEffect(() => {
    fetchbanner();
  }, []);
 ...
 //   handleImg: hàm xử lý sự kiện khi người dùng chọn một ảnh để tải lên.
//Sử dụng FileReader để đọc file và cập nhật state image.
